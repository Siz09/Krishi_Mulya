...
export const metadata: Metadata = {
  title: "Today's Fresh Fish Prices in Nepal | Kalimati Market",
  description: "Wholesale fresh fish prices today in Kalimati Fruits and Vegetable Market. Daily updates and historical trends.",
};
...
export default async function FishPage(props: PageProps) {
  ...
  // Fetch latest fish prices
  const prices = await getLatestPrices({ category: "fish", search });
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-8">
        ...
        {isStale && latestDateStr && (
          <div className="bg-soil-50 border border-leaf-100 rounded-xl p-4 flex items-start gap-3 shadow-sm">
            ...
            <p className="text-sm text-soil-800 font-medium">
              Notice: Showing fish prices for {formatBSDate(latestDateStr, "en")} ({latestDateStr}) as today's market prices have not yet been published by Kalimati.
            </p>
          </div>
        )}
      </header>
      ...
      <AlertSignupForm sourcePage="fish" />
    </div>
  );
}
